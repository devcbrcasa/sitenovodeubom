...
if not MONGO_URI:
    # Para teste local, você pode descomentar e definir sua URI aqui
    # ATENÇÃO: EM PRODUÇÃO NO NETLIFY, ISSO DEVE VIR DAS VARIÁVEIS DE AMBIENTE!
    # MONGO_URI = "mongodb+srv://cbr_records_db:eloah24@cluster0.c1z1ima.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
...
youtube_videos_collection = db.youtube_videos # Nova coleção para vídeos do YouTube

# Chave secreta para JWT (deve ser a mesma usada no seu login admin)
JWT_SECRET = os.environ.get('JWT_SECRET')
if not JWT_SECRET:
    # Para teste local, você pode descomentar e definir sua chave aqui
    # ATENÇÃO: EM PRODUÇÃO NO NETLIFY, ISSO DEVE VIR DAS VARIÁVEIS DE AMBIENTE!
    # JWT_SECRET = "tudocerto"
...
def change_password():
    ...
    if old_password == ADMIN_PASSWORD:
        # Em um ambiente real, você atualizaria o hash da senha no banco de dados
        # Por simplicidade, estamos apenas atualizando a variável de ambiente (não persistente)
        # Para persistência, você precisaria de um sistema de usuários no DB.
        os.environ['ADMIN_PASSWORD'] = new_password # Isso só funciona para a sessão atual da função
...
def update_portfolio_item(id):
    ...
    data = request.get_json()
    if not data:
        print("API: Nenhum dado fornecido para atualizar item de portfólio.")
        return jsonify({'message': 'Nenhum dado fornecido para atualização'}), 400
    try:
...
def handler(event, context):
    ...
    print(f"API: Resposta do Flask - Status: {response[0]}, Headers: {response[1]}")
    return {
...
if __name__ == '__main__':
    ...
    app.run(debug=True, port=8888) # Use uma porta diferente para evitar conflitos
